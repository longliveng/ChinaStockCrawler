...
class StockexchangeSpider(scrapy.Spider):
    ...
    def generateShanghai(self):
        ...
        self.myCursor.execute(
            "SELECT * FROM index_day_historical_data WHERE `type` = 000001")
        ...
        for stocklistkey in range(len(resultStockList)):
            ...
            if resultStockList[stocklistkey][14] is None or gapTimestamp > 259200 or resultStockList[stocklistkey][12] < 0:
                urlShanghai = 'http://query.sse.com.cn/marketdata/tradedata/queryTradingByProdTypeData.do?jsonCallBack=jsonpCallback63703&searchDate=' + str(
                    resultStockList[stocklistkey]
                    [2]) + '&prodType=gp&_=1485967505892'
                # urlShanghai='http://query.sse.com.cn/marketdata/tradedata/queryTradingByProdTypeData.do?jsonCallBack=jsonpCallback63703&searchDate='+'2017-01-26'+'&prodType=gp&_=1485967505892'
                # TODO 请求错误处理
    ...
    def generateShenzhen(self):
        ...
        self.myCursor.execute(
            "SELECT * FROM index_day_historical_data WHERE `type` = 399001")
        ...
        for stocklistkey in range(len(resultStockList)):
            ...
            if resultStockList[stocklistkey][14] is None or gapTimestamp > 259200 or resultStockList[stocklistkey][12] < 0:
                ...
                shenzhenFormdata['txtQueryDate'] = str(
                    resultStockList[stocklistkey][2])
    ...
    def indexDayParseShanghai(self, response):
        ...
        updateSql = "UPDATE `index_day_historical_data` SET `total_value` = " + str(
            resDict['result'][2]['marketValue1']
    ...
    def indexDayParseShenzhen(self, response):
        ...
        updateSql = "UPDATE `index_day_historical_data` SET `total_value` = '" + str(
            ...
            responseText
        ) + "',`update_time` = " + currentStamp + " WHERE `type` = 399001 and `date`='" + currDate + "'"
        # print updateSql
        # exit()
        self.myCursor.execute(updateSql)